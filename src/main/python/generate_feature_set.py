...
def generate_feature_set_descriptions():
    all_words = []
    for f in os.listdir('../resources/hydrated_tweets_descriptions/left'):
        with open('../resources/hydrated_tweets_descriptions/left/' + f, 'r', errors='ignore') as file_content:
            text = file_content.read()
        word_tokens = word_tokenize(text)
        filtered = [w for w in word_tokens if w not in stop_words]
        words = [w.lower() for w in filtered]
        all_words.append(words)
        all_words = flatten_lists(all_words)
    for f in os.listdir('../resources/hydrated_tweets_descriptions/right'):
        with open('../resources/hydrated_tweets_descriptions/right/' + f, 'r', errors='ignore') as file_content:
            text = file_content.read()
        word_tokens = word_tokenize(text)
        filtered = [w for w in word_tokens if w not in stop_words]
        words = [w.lower() for w in filtered]
